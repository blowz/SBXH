...
let tokenExpiry = 0;
// Promise 기반 락(Lock) 캐시 - 동시성 문제 완벽 해결
const folderLockCache = new Map();
...
async function fetchToken() {
    const config = (0,_config_js__WEBPACK_IMPORTED_MODULE_0__/* .getConfig */ .zj)(); // ◀ 이 줄로 수정
    ...
    return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            ontimeout: () => reject(new Error('Token request timed out (30s)'))
        });
    });
}
...
/**
 * Finds or creates a folder (동시성 락 적용 완벽 버전)
 */
async function getOrCreateFolder(folderName, parentId, token, category = 'Webtoon') {
    const cacheKey = `FOLDER_${parentId}_${category}_${folderName}`;

    // 1. 이미 동일한 폴더를 생성/조회 중이라면 중복 요청을 막고 끝날 때까지 대기합니다.
    if (folderLockCache.has(cacheKey)) {
        console.log(`[DirectUpload] 🚦 중복 폴더 생성 방지: 기존 생성 작업 대기 중... (${folderName})`);
        return await folderLockCache.get(cacheKey);
    }

    // 2. 비동기 작업 전체를 Promise Task로 묶어 캐시에 즉시 등록합니다.
    const folderTask = (async () => {
        const legacyQuery = `name='${folderName.replace(/'/g, "\\'")}' and '${parentId}' in parents and trashed=false and mimeType='application/vnd.google-apps.folder'`;
        const legacySearchUrl = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(legacyQuery)}&fields=files(id,name)&supportsAllDrives=true&includeItemsFromAllDrives=true`;

        const legacyResult = await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method: 'GET', url: legacySearchUrl,
        ...
        if (legacyResult.files && legacyResult.files.length > 0) return legacyResult.files[0].id;

        const categoryName = category || 'Webtoon';
        const catCacheKey = `CAT_${parentId}_${categoryName}`;
        let categoryFolderId;

        // 카테고리 폴더도 중복 생성 방지 락 적용
        if (folderLockCache.has(catCacheKey)) {
            categoryFolderId = await folderLockCache.get(catCacheKey);
        } else {
            const catTask = (async () => {
                const catQuery = `name='${categoryName.replace(/'/g, "\\'")}' and '${parentId}' in parents and trashed=false and mimeType='application/vnd.google-apps.folder'`;
                const categorySearchUrl = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(catQuery)}&fields=files(id,name)&supportsAllDrives=true&includeItemsFromAllDrives=true`;

                const categoryResult = await new Promise((resolve, reject) => {
                    GM_xmlhttpRequest({
                        method: 'GET', url: categorySearchUrl,
                        headers: { 'Authorization': `Bearer ${token}` },
                        timeout: 30000,
                        onload: (res) => { try { resolve(JSON.parse(res.responseText)); } catch (e) { reject(e); } },
                        onerror: reject
                ...
                if (categoryResult.files && categoryResult.files.length > 0) return categoryResult.files[0].id;

                const createCatResult = await new Promise((resolve, reject) => {
                    GM_xmlhttpRequest({
                        method: 'POST', url: 'https://www.googleapis.com/drive/v3/files?supportsAllDrives=true',
                        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                        data: JSON.stringify({ name: categoryName, mimeType: 'application/vnd.google-apps.folder', parents: [parentId] }),
                        timeout: 30000,
                        onload: (res) => { try { resolve(JSON.parse(res.responseText)); } catch (e) { reject(e); } },
                        onerror: reject
                    });
                });
                return createCatResult.id;
            })();
            folderLockCache.set(catCacheKey, catTask);
            categoryFolderId = await catTask;
        }

        const safeFolderName = folderName.replace(/'/g, "\\'");
        const seriesQuery = `name = '${safeFolderName}' and '${categoryFolderId}' in parents and trashed=false and mimeType='application/vnd.google-apps.folder'`;
        const seriesSearchUrl = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(seriesQuery)}&fields=files(id,name)&supportsAllDrives=true&includeItemsFromAllDrives=true`;

        const seriesResult = await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method: 'GET', url: seriesSearchUrl,
                headers: { 'Authorization': `Bearer ${token}` },
                timeout: 30000,
                onload: (res) => { try { resolve(JSON.parse(res.responseText)); } catch (e) { reject(e); } },
                onerror: reject
        ...
        if (seriesResult.files && seriesResult.files.length > 0) return seriesResult.files[0].id;

        const createResult = await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method: 'POST', url: 'https://www.googleapis.com/drive/v3/files?supportsAllDrives=true',
                headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                data: JSON.stringify({ name: folderName, mimeType: 'application/vnd.google-apps.folder', parents: [categoryFolderId] }),
                timeout: 30000,
                onload: (res) => { try { resolve(JSON.parse(res.responseText)); } catch (e) { reject(e); } },
                onerror: reject
            });
    ...
    folderLockCache.set(cacheKey, folderTask);

    try {
        return await folderTask; // 실제 구글 통신이 완료될 때까지 대기 후 ID 리턴
    } catch (error) {
        folderLockCache.delete(cacheKey); // 에러 시 캐시 롤백
        throw error;
    }
}

/**
 * Finds or creates the centralized '_Thumbnails' folder (동시성 락 적용 완벽 버전)
 */
async function getOrCreateThumbnailFolder(token, parentId) {
    const thumbName = '_Thumbnails';
    const cacheKey = `THUMB_${parentId}_${thumbName}`;
    ...
    const task = (async () => {
        ...
        const createResult = await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method: 'POST', url: 'https://www.googleapis.com/drive/v3/files?supportsAllDrives=true',
                headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                data: JSON.stringify({ name: thumbName, mimeType: 'application/vnd.google-apps.folder', parents: [parentId] }),
                timeout: 30000,
                onload: (res) => resolve(JSON.parse(res.responseText)),
                onerror: reject
            });
    ...
    folderLockCache.set(cacheKey, task);

    try {
        return await task;
    } catch (e) {
        folderLockCache.delete(cacheKey);
        throw e;
    }
}
/**
 * Sends data in chunks to a Google Drive Resumable Upload session
 * [수정됨] 503 에러 발생 시 자동 대기 및 재시도 로직 추가
 */
async function sendResumableChunks(uploadUrl, blob, token, fileName) {
    ...
    while (start < totalSize) {
        ...
        let chunkRetries = 3; // 구글 서버 에러 시 최대 3회 재시도
        ...
        while (chunkRetries > 0 && !chunkSuccess) {
            try {
                    ...
                    GM_xmlhttpRequest({
                        ...
                        onload: (res) => {
                            if (res.status === 308 || (res.status >= 200 && res.status < 300)) {
                                ...
                                // 구글 서버 일시적 과부하 (Rate Limit) 에러 발생
                                reject(new Error(`RATE_LIMIT:${res.status}`));
                            } else {
                ...
                chunkSuccess = true; // 통과 시 루프 탈출
            } catch (e) {
                if (e.message.startsWith('RATE_LIMIT')) {
                    chunkRetries--;
                    if (chunkRetries > 0) {
                        logger.log(`[Drive] 구글 서버 과부하 감지. 3초 대기 후 재시도합니다... (남은 재시도: ${chunkRetries}회)`);
                        // 3초 대기 (구글 서버가 숨 돌릴 시간 제공)
                        await new Promise(r => setTimeout(r, 3000));
                    } else {
                        // 3번 다 실패하면 기존처럼 에러 던지고 폴백으로 넘김
                        throw new Error(`Chunk upload failed: 503 Service Unavailable`);
                    }
                } else {
                    throw e; // 다른 심각한 에러는 즉시 던짐
                }
            }
        }
...
async function uploadDirect(blob, folderName, fileName, metadata = {}) {
    try {
        ...
        try {
            // [Fix] 파일명 내 홑따옴표(') 처리 및 전체 검색 쿼리 주소 깨짐 우회 인코딩 반영
            const query = `name='${finalFileName.replace(/'/g, "\\'")}' and '${targetFolderId}' in parents and trashed=false`;
            const searchUrl = `https://www.googleapis.com/drive/v3/files?` +
...
async function uploadViaGASRelay(blob, folderName, fileName, options = {}) {
    ...
    // Determine Category
    let category = options.category || (fileName.endsWith('.epub') ? 'Novel' : 'Webtoon');

    // 썸네일 중앙 관리 분기 로직 추가 (완벽 방어)
    ...
    if (fileName === 'cover.jpg' || fileName === 'Cover.jpg') {
        ...
        if (idMatch) {
            const seriesId = idMatch[1];
            finalFileName = `${seriesId}.jpg`;   // 파일명을 작품번호.jpg로 변경
            finalFolderName = '_Thumbnails';      // 폴더명을 _Thumbnails로 고정
            category = '';                        // 카테고리를 비워 최상위 폴더에 생성 유도
        }
    }
    ...
    await new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            data: JSON.stringify({
                ...
                folderName: finalFolderName, // 수정된 폴더명 적용
                fileName: finalFileName,     // 수정된 파일명 적용
                category: category,
                apiKey: config.apiKey
            }),
            headers: { "Content-Type": "text/plain" },
            ...
            onload: (res) => {
                try {
                    ...
                    if (json.status === 'success') {
                        ...
                        logger.critical(`GAS 릴레이 세션 초기화 실패: ${json.body || 'Init failed'} (${finalFileName})`, 'GAS:Relay');
            ...
            onerror: (e) => {
                logger.critical(`GAS 릴레이 네트워크 오류 (Init) — ${finalFileName}`, 'GAS:Relay');
                reject(new Error("네트워크 오류(Init)"));
            },
            ontimeout: () => {
                logger.critical(`GAS 릴레이 세션 초기화 타임아웃 (30초) — ${finalFileName}`, 'GAS:Relay');
    ...
    while (start < totalSize) {
        ...
        await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                ontimeout: () => reject(new Error(`[GAS] 청크 업로드 타임아웃: ${start}~${end}`))
            });
        });
...
// 🚀 새롭게 추가된 AES-GCM 복호화 (shadow-v3 전용)
async function aesGcmDecrypt(payloadB64, cookieKeyB64, novelId, episodeId) {
    const rawPayload = b64urlDecode(payloadB64);
    if (rawPayload.length < 28) throw new Error("페이로드 데이터가 너무 짧습니다.");

    const iv = rawPayload.slice(0, 12);
    const ciphertext = rawPayload.slice(12);

    const cookieBytes = b64urlDecode(cookieKeyB64);
    const suffixBytes = new TextEncoder().encode(":" + novelId + ":" + episodeId + ":v3");

    const keyMaterial = new Uint8Array(cookieBytes.length + suffixBytes.length);
    keyMaterial.set(cookieBytes, 0);
    keyMaterial.set(suffixBytes, cookieBytes.length);
...
async function fetchNovelText(episodeUrl, config = {}, retryCount = 0) {
    ...
    const uiLog = (msg) => {
        try {
            const ul = document.getElementById('toki-logbox-content');
            if (ul) {
                const li = document.createElement('li');
                li.className = 'warn';
                const time = new Date().toLocaleTimeString('ko-KR', { hour12: false });
                li.textContent = `[${time}] [Decryptor] ${msg}`;
                ul.appendChild(li);
                ul.scrollTop = ul.scrollHeight;
            }
            console.warn(`[Decryptor] ${msg}`);
        } catch(e) {}
    };

    try {
        const ids = getIdsFromUrl(episodeUrl);
        if (!ids) throw new Error("URL에서 작품/에피소드 ID를 찾을 수 없습니다.");

        const respHtml = await fetch(episodeUrl, { credentials: 'same-origin' });
        if (!respHtml.ok) throw new Error(`HTML 로드 실패 (HTTP ${respHtml.status})`);
        const html = await respHtml.text();

        const tokenMatch = html.match(RE_TOKEN);
        if (!tokenMatch) throw new Error("보안 토큰 추출 실패 (Cloudflare 차단 의심)");
        ...
        if (!cookie) throw new Error("복호화 쿠키 발급 실패");
        ...
        if (!resp.ok || !data || !data.ok || !data.payload) {
            throw new Error(`API 응답 데이터 없음 (HTTP ${resp.status})`);
        }

        // 6. AES-GCM 복호화 진행 (v3)
        const rawText = await aesGcmDecrypt(data.payload, xorKey, ids.novelId, ids.episodeId);
        if (!rawText) return null;

        // 7. JSON 파싱 및 셔플(퍼즐) 해제
        try {
            const dataObj = JSON.parse(rawText);

            if (dataObj.kind === 'text-shuffled' && Array.isArray(dataObj.paragraphs) && Array.isArray(dataObj.perm)) {
                const unshuffled = new Array(dataObj.paragraphs.length);
                for (let i = 0; i < dataObj.paragraphs.length; i++) {
                    unshuffled[dataObj.perm[i]] = dataObj.paragraphs[i];
                }
                return unshuffled.join('\n\n');
            } else if (dataObj.kind === 'text' && Array.isArray(dataObj.paragraphs)) {
                return dataObj.paragraphs.join('\n\n');
            } else if (dataObj.kind === 'html' && dataObj.html) {
                return dataObj.html;
            } else if (dataObj.text) {
                return dataObj.text;
            }
        } catch (e) {
            console.warn("[Decryptor] JSON 파싱 실패", e);
            return rawText;
        }

    } catch (e) {
        if (retryCount < MAX_RETRIES) {
            uiLog(`소설 서버 에러 감지(${e.message}) → 3초 대기 후 재시도 (${retryCount + 1}/${MAX_RETRIES})`);
            await resetNvCookie(cookieName);
            await new Promise(r => setTimeout(r, 3000));
            return fetchNovelText(episodeUrl, config, retryCount + 1);
        }
        uiLog(`최대 재시도 횟수(${MAX_RETRIES}) 초과. 해당 에피소드는 실패 처리됩니다.`);
        return null;
    }
}
/***/ }),
...
class TxtBuilder {
    ...
    addChapter(title, textContent) {
        this.content += `\n\n=== ${title} ===\n\n`;
        // 👇 textContent를 cleanNovelParagraphs로 감싸도록 수정 👇
...
async function processItem(item, builder, siteInfo, iframe, parser, seriesTitle = "", targetDoc = null) {
    ...
    if (fetchMethod === 'api') {
        ...
        if (text) {
            ...
            // [추가된 부분] API 실패 시 빈 페이지인지 한 번 더 확인 후 스킵
            const checkHtml = await fetch(item.src).then(r=>r.text());
            if (checkHtml.includes("본문이 아직 준비되지 않았습니다") || checkHtml.includes("novel-empty")) {
                logger.warn(`본문이 준비되지 않은 회차 (자동 건너뜀): ${item.title}`, 'Downloader');
                // return 대신 에러를 던져서 파일(epub) 생성 및 업로드 단계를 강제로 취소시킵니다.
                throw new Error("본문이 준비되지 않은 회차 (다운로드 취소됨)");
            }
    ...
    if (isNovel) {
        ...
        if (suspiciousCount > mergedUrls.length / 2) {
            ...
            // [Fix] 재수집한 데이터 덩어리에서 순수 URL 글자만 뽑아내도록 수정 (.map 적용)
            images = await fetchImages(finalRetryUrls.map(u => u.url).filter(u => u !== ""));
        }
...
async function tokiDownload(rangeSpec, policy = 'zipOfCbzs', forceOverwrite = false) {
    ...
    try {
        ...
        // [Fix] "준비중" (미오픈) 에피소드 자동 스킵 필터링 및 로그 알림 추가
        const skippedEpisodes = [];
        list = list.filter(li => {
            const el = li.element || li;
            if (!el) return true;

            const isNotReadyClass = el.classList && el.classList.contains('novel-ep--not-ready');
            const text = el.innerText || el.textContent || "";
            const isNotReadyText = text.includes('⏳ 준비중') || text.includes('준비중');

            if (isNotReadyClass || isNotReadyText) {
                try {
                    // 제외되는 에피소드의 제목을 깔끔하게 추출해서 배열에 담습니다.
                    const parsedItem = parser.parseListItem(el);
                    skippedEpisodes.push(parsedItem.title);
                } catch(e) {
                    skippedEpisodes.push("알 수 없는 에피소드");
                }
                return false;
            }

            return true; // 오픈된 정상 에피소드만 통과
        });

        // 제외된 에피소드가 있다면 로그창에 노란색(Warn)으로 출력해 줍니다.
        if (skippedEpisodes.length > 0) {
            logger.warn(`⏳ 준비중(미오픈) 에피소드 ${skippedEpisodes.length}개 제외됨:\n - ${skippedEpisodes.join('\n - ')}`, 'Downloader');
        }
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            try {
                ...
                // [추가된 부분] 의도된 스킵(취소)인 경우 실패 리포트에 기록하지 않고 조용히 패스
                if (errorMsg.includes("다운로드 취소됨")) {
                    // (리포트 생성 방지) 아무 작업도 하지 않음
                } else {
                    // 진짜 에러일 때만 로그 띄우고 리포트에 추가
                    console.error(err);
...
async function main() {
    ...
    if (typeof GM_registerMenuCommand !== 'undefined') {
        ...
        GM_registerMenuCommand('🌐 뷰어 열기', openViewer);
        ...
        window.addEventListener('message', (event) => {
            ...
            if (msg.type === 'TOKI_API_REQUEST') {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: (response) => {
                        try {
                            // [추가된 부분] 본문이 없는 빈 회차인 경우 에러를 뱉어서 다운로드 건너뛰기
                            if (response.responseText.includes("본문이 아직 준비되지 않았습니다") || response.responseText.includes("novel-empty")) {
                                window.postMessage({
                                    type: 'TOKI_API_RESPONSE',
                                    requestId: msg.requestId,
                                    error: "본문이 준비되지 않은 회차 (자동 건너뜀)"
                                }, '*');
                                return;
                            }
