    ...
    return new Promise((resolve) => {
        ...
        try {
            if (activePopupRef && !activePopupRef.closed) {
                ...
                const halfHeight = Math.floor(window.screen.availHeight / 2);
                activePopupRef = window.open(episodeUrl, 'tokisync-novel-worker', `width=${halfWidth},height=${halfHeight},left=0,top=0,noopener=false`);
                if (!activePopupRef) {
...
async function processItem(item, builder, siteInfo, iframe, parser, seriesTitle = "", targetDoc = null) {
    ...
    const isWebtoon = (category === 'Webtoon' || category === 'webtoon');
    const viewerCfg = parser.rule.viewer || {};
    ...
    if (isNovel) {
        ...
        if (isWebtoon) {
            logger.log(`[웹툰] 추출 중: ${item.title}`, 'Downloader');
        } else {
